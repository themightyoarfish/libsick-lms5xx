...
static std::string method(const char *sopas_reply, size_t len) {
  if (len < sizeof("\x02...")) {
...
static bool status_ok(const std::string &cmd_name, int status_code) {
...
static sick_err_t status_from_bytes_ascii(const char *data, size_t len) {
