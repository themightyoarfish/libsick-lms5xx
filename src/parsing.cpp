    ...
    if (time_exists == 1) {
        ...
        if (channels_16bit.size() < 1) {
            ...
            if (range_cn.description.find("DIST") == std::string::npos) {
                throw std::runtime_error(
                    "First 16bit channel was not range but " +
                    range_cn.description);
            } else {
                const Channel &intensity_cn = channels_8bit.front();
                if (intensity_cn.description.find("RSSI") ==
                    std::string::npos) {
                    throw std::runtime_error(
                        "First 8bit channel was not intensity but " +
                        range_cn.description);
                } else {
                    if (range_cn.values.size() != intensity_cn.values.size()) {
                        ...
                        if (scan.ranges.size() == 0) {
                            ...
                            scan.intensities =
                                Eigen::VectorXf::Zero(scan.size, 1);
                            scan.ang_increment = range_cn.ang_incr;
                            scan.start_angle =
                                angle_to_lms(range_cn.angles.front());
                            scan.end_angle =
                                angle_to_lms(range_cn.angles.back());
                        ...
                        std::memcpy(scan.ranges.data(), &range_cn.values[0],
                                    scan.size * sizeof(float));
                        scan.ranges /= 1000;
                        std::memcpy(scan.intensities.data(),
                                    &intensity_cn.values[0],
                                    scan.size * sizeof(float));
                        scan.time = stamp;
                        return true;
                    }
...
std::string method(const char *sopas_reply, size_t len) {
    if (len < sizeof("\x02...")) {
        throw std::runtime_error(
            "Sopas reply string too short to have a method.");
    } else
